...
if [ -f "$PROJECT_ROOT/services/raport-service/migration/create_tables.sql" ]; then
    podman exec -i postgres-raports psql -U postgres -d reports_management < "$PROJECT_ROOT/services/raport-service/migration/create_tables.sql"
    if [ $? -eq 0 ]; then
